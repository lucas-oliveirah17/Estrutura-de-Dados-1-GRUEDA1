...
int removeInicio(Lista *li){
    ...
    /*Resolve os dois casos, se a lista vai ficar vazia
    ou não*/
    *li = no->prox;
    free(no);
    return matricula;
...
int removeFinal(Lista *li){
    ...
    ELEM *ant, *no = *li;
    /*Percorre a lista até o final, procurando o último
    elemento.*/
    while(no->prox != NULL){
        ant = no;
        no = no->prox;
    ...
    if(no == (*li)){
        *li  = no->prox;
        ...
        /*O nó ant-prox passa a apontar para onde o no->prox
        aponta.*/
        ant->prox = no->prox;
    }
    matricula = no->dados.matricula;
    free(no);
    /*Devolve matrícula do aluno removido*/
    return matricula;
}
